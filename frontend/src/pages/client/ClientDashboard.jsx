...
export default function ClientDashboard() {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-300">
      {/* Top Welcome Banner */}
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 bg-gradient-to-r from-slate-800 via-slate-800/80 to-slate-900 p-6 rounded-3xl border border-slate-700/80 shadow-xl">
        <div>
        ...
        <div className="flex items-center space-x-3">
          ...
          <Link
            to="/transfer"
            className="flex items-center space-x-2 px-4 py-2.5 bg-slate-700/80 hover:bg-slate-700 text-slate-200 text-sm font-semibold rounded-xl border border-slate-600 transition"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-slate-800/80 p-6 rounded-3xl border border-slate-700/80 shadow-xl">
          ...
          <p className="mt-3 text-xs text-slate-400">
            Savings, Business & Checking accounts
          </p>
        </div>
        ...
        <div className="bg-slate-800/80 p-6 rounded-3xl border border-slate-700/80 shadow-xl">
          ...
          <p className="mt-3 text-xs text-slate-400">
            Active session monitor with auto-renewal
          </p>
        </div>
      </div>
            ...
            {accounts.map((acc) => (
              ...
              >
                ...
                <div className="pt-4 border-t border-slate-700/60 flex items-center justify-between text-xs">
                  {acc.account_type === 'Savings' && (
                    <span className="text-slate-400">Min. Bal: ${acc.minimum_balance} | 5% APY</span>
                  )}
                  {acc.account_type === 'Business' && (
                    <span className="text-slate-400">Fee: ${acc.transaction_fee} / withdrawal</span>
                  )}
